...
//  VARIABLE memvar 
//  0 memvar !
//  : mem-access 32000 0 DO memvar @ I + memvar ! memvar @ I - memvar ! LOOP ;
//  : mem-nested 32000 0 DO mem-access LOOP ;
//  : mem-test mem-nested memvar @ ." final should be 0: " . CR ;
// mem-test will execute 1024000000 total loops
// When I first time compiled the release version (still 16-bit, but for 64-bit target!),
// it took ~39 seoconds to complete this test.
// After optimising several primitives the result is ~26 seconds!

// Swap test:
// : swap-10 1 2 32000 0 DO SWAP SWAP SWAP SWAP SWAP SWAP SWAP SWAP SWAP SWAP LOOP DROP DROP ;
// : swap-test 32000 0 DO swap-10 LOOP ." SWAP TEST DONE!" CR ;
// Before: ~43 seconds, After: ~28 sec
// --VH

#define TRUE  (1)   // TRUE is old :)
#define FALSE (0)
...
#define CO (KBBUFF+4)   /* size of a disk buffer w/4 words overhead */
...
#define USER_DEFAULTS (ORIGIN+SCRATCHSIZE)  /* 16 */
                /* start of user variable initial-values space -- it's DEFS-SIZE bytes long */
...
#define INITS0 (TIB_START+KBBUFF+CS_SIZE) /* c. stack grows down CSS words, into end of TIB */
#define INITR0 (INITS0+RS_SIZE)	/* Return stack grows down RSS words, bangs into INITS0. */
...
#define SAVEDIP 2 /* mem[SAVEDIP] = 0 for newly-generated systems, or the IP for a saved system */
